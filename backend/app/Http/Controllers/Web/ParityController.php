...
class ParityController extends Controller
{
    public function toggleFavorite(Request $request, Product $product): RedirectResponse
    {
        ...
        if ($favorite) DB::table('favorites')->where('id', $favorite->id)->delete();
        else DB::table('favorites')->insert(['user_id' => $request->user()->id, 'product_id' => $product->id, 'created_at' => now(), 'updated_at' => now()]);
    ...
    public function screen(Request $request, string $area, string $screen): Response
    {
        ...
        if ($key === 'user/categories' && Schema::hasTable('categories')) {
            ...
            $id = $request->user()->id;
            $payload['rows'] = Conversation::where(fn ($q) => $q->where('participant_one_id', $id)->orWhere('participant_two_id', $id))->with(['participantOne:id,name', 'participantTwo:id,name', 'product:id,title'])->withCount('messages')->latest('last_message_at')->limit(50)->get()->map(function ($c) use ($id) { $other = (int) $c->participant_one_id === (int) $id ? $c->participantTwo : $c->participantOne; return ['title' => $other?->name ?: 'Conversation', 'meta' => ($c->product?->title ?: 'Marketplace chat').' · '.$c->messages_count.' messages', 'href' => '/chats/'.$c->id]; })->all();
        ...
        if ($payload['rows'] === []) $payload['empty'] = 'No records found yet.';
