...
class MarketplaceController extends Controller
{
    ...
    public function products(Request $request): Response
    {
        ...
        if ($request->filled('q')) $query->where(fn ($q) => $q->where('title', 'like', '%'.$request->string('q').'%')->orWhere('brand', 'like', '%'.$request->string('q').'%')->orWhere('description', 'like', '%'.$request->string('q').'%'));
        if ($request->filled('brand')) $query->where('brand', 'like', '%'.$request->string('brand').'%');
        if ($request->filled('category')) $query->whereHas('category', fn ($q) => $q->where('slug', $request->string('category')));
        if ($request->filled('shop')) $query->whereHas('shop', fn ($q) => $q->where('name', 'like', '%'.$request->string('shop').'%')->orWhere('address', 'like', '%'.$request->string('shop').'%'));
        if ($request->boolean('verified')) $query->whereHas('shop', fn ($q) => $q->where('verified', true));
        if ($request->filled('condition') && in_array($request->string('condition')->toString(), ['new', 'used', 'refurbished'], true)) $query->where('condition', $request->string('condition'));
